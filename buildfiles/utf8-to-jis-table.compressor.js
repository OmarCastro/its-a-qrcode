...
export function toTableMap () {
  ...
  const result = {}
  const nodesToAdd = []
  for (const key of Object.keys(inverseTable)) {
    if (visitedValues.has(key)) {
      continue
    }
    ...
    result[keyNum.toString(16)] = [...nodesToAdd, parseInt(inverseTable[keyNum]).toString(16)].reverse().join(',')
    nodesToAdd.length = 0
  }
  return result
}

export function toCompressedMap () {
  const tableMap = toTableMap()
  for (const [key, value] of Object.entries(tableMap)) {
    ...
    for (const valStr of value.split(',')) {
      if (valStr.includes(':')) {
        if (acc !== 0) {
          ...
          values.push(`${currentIt.toString(16)}`)
        }
        values.push(valStr)
        currentIt = null
        acc = 0
      ...
      const valInt = parseInt(valStr, 16)
      if (currentIt == null) {
        currentIt = valInt
        continue
      }
      if (currentIt + acc + 1 === valInt) {
        acc++
        continue
      }
      if (acc !== 0) {
    ...
    tableMap[key] = values.join(',')
  }
  return tableMap
}

const xTob64 = hexToBase64
export function tob64CompressedMap () {
  const tableMap = toCompressedMap()
  const result = {}
  for (const [key, value] of Object.entries(tableMap)) {
    const values = []
    for (const valStr of value.split(',')) {
      ...
      if (valStr.includes('>')) {
        const [val, it] = valStr.split('>')
        values.push(`${xTob64(val)}>${it}`)
        continue
      }
      values.push(xTob64(valStr))
    }
...
function getNonCompressedTable () {
  return {
