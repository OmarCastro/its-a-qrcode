import { sjisBytesToText, bytesToText, textToBytes, bytesToHex, hexToBytes } from '../src/utils/text-decode-encode.util.js'
import pkg from 'deep-diff'
const { diff } = pkg

/**
 * @typedef {object} Utf8ToJisTableEntry
 * @property {number} utf8 - UTF8 code
 * @property {number} sjis - SHIFT-JIS code
 * @property {number} index - SHIFT-JIS index
 * @property {Uint8Array} sjisBytes - SHIFT-JIS code as a byte array
 */
/** @type {{[utf8: number]: Utf8ToJisTableEntry}} */
let tableMemo
/**
 *
 */
function createUtf8ToSJisTable () {
  ...
  /** @type {Map<number, Utf8ToJisTableEntry>} */
  const map = new Map()
  const ranges2 = [[0x21, 0x5F], [0x2121, 0x7C80], [0x1215D, 0x1215D]]
  const convertMap = Object.fromEntries(Object.entries(additionalUtf8ToSjisIndexEntries).map(([k, v]) => [v, parseInt(k)]))

  for (const [rangeStart, rangeEnd] of ranges2) {
    for (let index = rangeStart; index <= rangeEnd; index++) {
      const lastByte = (index & 0xFF)
      if (lastByte >= 0x7F || lastByte < 0x21) { continue }
      const sjis = sjisIndexToCode(index)
      let hex = sjis.toString(16)
      if (hex.length & 1) { hex = '0' + hex }
      const sjisBytes = Uint8Array.from(hexToBytes(hex))
      const text = sjisBytesToText(sjisBytes)
      const utf8Bytes = textToBytes(text)
      const utf8 = parseInt(bytesToHex(utf8Bytes), 16)
      if (utf8 === 0xEFBFBD) { continue }
      const value = { utf8, sjis, index, sjisBytes }
      if (!map.has(utf8)) {
        map.set(utf8, value)
      }
      const additionalUtf8 = convertMap[index]
      if (additionalUtf8 && !map.has(additionalUtf8)) {
        map.set(additionalUtf8, value)
      }
    }
  }
...
const table = createUtf8ToSJisTable()

console.log(diff(Object.fromEntries(Object.entries(table).map(([k,v])=>[k,v.index])), getNonCompressedTable()))

let hex1 = 0xC2AC.toString(16)
if (hex1.length & 1) { hex1 = '0' + hex1 }
let hex2 = sjisIndexToCode(0x224C).toString(16)
if (hex2.length & 1) { hex2 = '0' + hex2 }
const a = Uint8Array.from(hexToBytes(hex2))
const b = sjisBytesToText(a)
const c = textToBytes(b)
const d = bytesToHex(c)

const a1 = Uint8Array.from(hexToBytes(hex1))
...
export function getNonCompressedTable () {
  return {
    ...
    0xEFBD97: 0x2377, 0xEFBD98: 0x2378, 0xEFBD99: 0x2379, 0xEFBD9A: 0x237A, 
    ...
    0xE38292: 0x2472, 0xE38293: 0x2473, 
    
    0xE382A1: 0x2521, 0xE382A2: 0x2522, 0xE382A3: 0x2523,
    ...
    0xE383B4: 0x2574, 0xE383B5: 0x2575, 0xE383B6: 0x2576, 
