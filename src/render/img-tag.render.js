 ...
 */
export function createImgTag ({ cellSize, margin, alt, qrcode, colors }) {
  cellSize ||= 2
  margin ??= cellSize * 4
  const size = qrcode.moduleCount * cellSize + margin * 2
  const altAttr = alt ? ` alt="${escapeXml(alt)}"` : ''
  return `<img src="${createDataURL({ cellSize, margin, qrcode, colors })}" width="${size}" height="${size}"${altAttr}/>`
};
